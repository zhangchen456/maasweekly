/**
 * http.ts：路由、ETag/304、CORS、Problem JSON、限流（Task 03 M5）。
 * Node 22 原生 http，零运行时依赖。单请求单 Dataset 实例（重载不混版）。
 */
...
function problem(res: ServerResponse, requestId: string, p: Problem, extra?: Record<string, string>): void {
...
function notFound(res: ServerResponse, requestId: string, detail: string): void {
...
function envelope(ds: Dataset, query: Record<string, unknown>, coverage: Record<string, unknown>): {
  schemaVersion: string; datasetVersion: string; dataThrough: string;
    ...
    if (seg[0] === 'api' && seg[1] === 'v1') {
      ...
      try {
        ...
        switch (rest[0]) {
          ...
          case 'weekly':
            ...
            if (rest.length === 2) {
              ...
              if (!w) { notFound(res, requestId, `未知周报: ${rest[1]}`); return; }
          ...
          case 'items':
          case 'evidence': {
            if (rest.length !== 2) { notFound(res, requestId, '需要 ID'); return; }
            const id = rest[1]!;
        ...
        problem(res, requestId, {
          ...
          detail: (e as Error).message, code: 'internal_error',
          recovery: '稍后重试。',
        });
        return;
      }
