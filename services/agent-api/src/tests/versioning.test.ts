...
test('T12 翻页期间发布新版本：旧 cursor 继续读旧 release', async () => {
  ...
  const dNew = await rNew.json() as { datasetVersion: string };
  ...
  const d2 = await r2.json() as { datasetVersion: string; items: { title: string }[] };
...
test('T12c 篡改 cursor → 400 invalid_cursor', async () => {
  ...
  const d2 = await r2.json() as { page: { nextCursor: string } };
  ...
  assert.equal((await r3.json() as Record<string, string>)['code'], 'invalid_cursor');
  // 坏 base64
...
test('T15 热重载：成功原子切换 / 损坏版本续服旧版 / 单请求不混版', async () => {
  ...
  const d = await r.json() as { datasetVersion: string };
  assert.equal(d.datasetVersion, V2);
  // status 披露错误
  const rs = await get('/api/v1/status');
  const ds = await rs.json() as { status: { service: { lastReloadError: string | null } } };
