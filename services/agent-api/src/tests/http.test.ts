...
async function get(p: string, headers: Record<string, string> = {}): Promise<Response> {
...
test('T06 changes 默认查询（dataThrough 锚定 7 天窗口）', async () => {
  ...
  const q = d.query as Record<string, string>;
  assert.equal(q.from, '2026-09-10');
  assert.equal(q.to, '2026-09-17'); // [from, to) 语义：to = D+1
  const items = d.items as { observationDate: string }[];
...
test('T09 item/evidence/weekly 实体与未知 404', async () => {
  ...
  const dd = await rid.json() as { items: Record<string, unknown>[] };
  const id = dd.items.find((i) => i['recordType'] === 'price_change')!['id'] as string;
  const r = await get(`/api/v1/items/${id}`);
  assert.equal(r.status, 200);
  const item = (await r.json() as Record<string, unknown>)['item'] as Record<string, unknown>;
  assert.ok(Array.isArray(item['revisionHistory']));
  // evidence 引用闭环
  const eid = (item['evidenceIds'] as string[])[0]!;
  const re = await get(`/api/v1/evidence/${eid}`);
  assert.equal(re.status, 200);
  const ev = (await re.json() as Record<string, unknown>)['item'] as Record<string, unknown>;
  ...
  const dw = await rw.json() as { items: { id: string }[] };
...
test('T11 同版本翻完全部页（limit=1，无重复无遗漏）', async () => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    assert.equal(r.status, 200);
    const d = await r.json() as { items: { id: string }[]; page: { nextCursor: string | null } };
    seen.push(...d.items.map((x) => x.id));
    if (!d.page.nextCursor) break;
