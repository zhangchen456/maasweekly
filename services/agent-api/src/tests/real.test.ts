...
test('T18 六类核心查询返回真实数据', async () => {
  ...
  const d1 = await r1.json() as { items: unknown[]; datasetVersion: string; query: Record<string, string> };
  ...
  const d6 = await r6.json() as { status: { providers: unknown[]; priceStreams: unknown[] } };
...
test('T18 公开引用零悬空（抽样 200 条）', async () => {
  ...
  const d = await r.json() as { items: { evidenceIds: string[]; links: { permalink: string } }[] };
  ...
  const dp = await rp.json() as { items: { evidenceId: string | null }[] };
...
test('T18 性能与元数据一致性', async () => {
  ...
  for (const p of ['/api/v1/changes?limit=1', '/api/v1/prices?limit=1',
    ...
    const d = await (await get(p)).json() as { datasetVersion: string };
