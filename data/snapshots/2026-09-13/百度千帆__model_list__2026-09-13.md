<!-- url: see sources config -->
<!-- fetched: 2026-09-13T06:51:25.543500 -->

模型列表 - 百度千帆·大模型服务及Agent开发平台
百度智能云
备案
文档
管理控制台
This app works best with JavaScript enabled.
文档中心
搜索本产品文档关键词立即试用
模型列表
百度千帆·大模型服务及Agent开发平台
平台介绍
平台简介
更新动态
产品公告
模型服务
模型更新记录
平台更新记录
模型版本升级及退役机制
Token Plan个人版上线及Coding Plan停售公告
Coding Plan 产品升级公告
批量推理促销活动说明
Anthropic API 返回参数input_tokens含义更新
7月28日服务停服通知
模型服务 V1 接口下线及迁移说明
应用开发
模型赠送资源下线公告
Agent模型服务升级及切换公告
知识库高级解析服务计费公告
共享资源知识库检索计费公告
工具及MCP广场
官方工具下架公告
工具限时体验额度到期通知
教育类工具下架公告
热榜类工具下架公告
模型介绍
快速开始-模型服务调用
模型列表
功能特性
模型能力
模型默认参数说明
Token Plan 个人版
Token Plan 个人版
快速开始
接入AI工具
OpenClaw
Claude Code
OpenCode
Cursor
CodeX
Qwen Code
Kilo CLI
Kilo Code
Cline
Cherry Studio
CC-Switch
Hermes Agent
DeepSeek Harness
ZCode
Coding Plan 升级与权益调整说明
Coding Plan 迁移至 Token Plan 用户授权协议
使用须知
Token Plan 企业版
Token Plan 企业版
快速开始
团队管理
接入工具
OpenCode
CC-Switch
Qwen Code
Kilo CLI
Kilo Code
Cherry Studio
Cline
CodeX
Cursor
Claude Code
OpenClaw
Zcode
Hermes Agent
DeepSeek Harness
Coding Plan
Coding Plan
接入AI工具
OpenClaw
Claude Code
OpenCode
Cursor
Cline
CodeX
Qwen Code Companion
Cherry studio
Kilo CLI
Kilo Code
cc-switch
计费说明
模型服务
模型服务计费
价格详情
数据管理
模型精调
模型蒸馏
模型评估
续费和退款说明
退款申请说明
续费和自动续费
token计算说明
模型token计算说明
通过cfc发布token计算在线API
新用户免费额度
成本拆分
自定义接入点（Endpoint）成本拆分迁移指南
成本拆分使用说明
官方工具
百度AI搜索
智能搜索生成
智能搜索生成高性能版
百度搜索
相似图搜索
API Key维度分账
QPS叠加包
应用开发
Agent开发平台
官方Agent
深度研究Agent
AI助手
Token 福利包
Token 福利包
接入工具
Claude Code
OpenClaw
OpenCode
Cursor
CodeX
Cline
Qwen Code
Cherry Studio
Kilo Code
Kilo CLI
CC-Switch
知识库
最佳实践
模型服务
模型管理
HuggingFace模型下载、部署、调用实战
模型训练
文生文SFT最佳实践
图像理解SFT最佳实践
Post-pretrain最佳实践
生态集成
通过服务网卡私网访问千帆模型服务
批量推理
Files + Batch 接口使用指南（OpenAI 兼容版）
应用开发
官方模板介绍
数字人应用：创建、调试和SDK集成
数字人应用：美食顾问
记忆变量：MBTI性格测试助手
自主规划Agent最佳实践
用户旅程地图生成
自主规划Agent：开发AI手机实战，让手机开启「自动驾驶」
自主规划Agent：营销策略设计师
自主规划Agent一键发布到秒哒：茶叶销售线上转型实践
工作流Agent最佳实践
工作流Agent：“金牌销售”车险续保Agent
工作流Agent：如何使用全局跳转节点搭建飞行客服小助手
工作流Agent：机票订单处理助手
工作流大模型节点高级配置使用案例
多智能体协同Agent最佳实践
用“多智能体协同Agent”一分钟搭建专属研究Agent，帮企业做深度研究
自定义工具最佳实践
工作流组件： API的增删查
工作流组件：如何用知识库节点和API节点搭建个人减脂小助手
工作流组件：深度使用实践
组件工作流：使用记忆变量节点构建个人化导购小助手
知识库最佳实践
图谱增强RAG：医疗诊断助手
多模态RAG：图文问答助手
对象存储（BOS）与向量检索(BES)：企业级知识库创建
检索增强生成（RAG）应用：提示词调试
航司行李规格查询助手
表格型知识库与数据库：课程信息检索
卡片功能最佳实践
智能菜谱推荐应用搭建
官方工具
百度AI搜索
百度AI搜索应用搭建
百度搜索组件：一文读懂多种搜索组合方式
如何在OpenClaw调用官方工具
OpenClaw最佳实践
冬奥智能解说助手
我的职场PPT生成助手—热点追踪
孩子成长记录绘本助手
平台操作
模型服务
快速开始-平台功能使用
体验中心
模型服务
在线服务
批量推理
推理日志投递与解绑
推理服务监控告警
自定义接入点
模型管理
我的模型
HuggingFace格式模型导入
将HuggingFace模型下载到BOS
模型创建与管理
模型评估
效果评估
产品描述
自动评估
创建自动评估任务
查看与管理自动评估任务
人工评估
创建人工评估任务
查看与管理人工评估任务
性能评估
模型压缩
数据管理
推理结果集
使用推理结果集
创建推理结果集
创建推理结果集任务
通用数据集
推理日志挖掘
数据标注
众测标注
在线标注
多人标注
数据洞察与处理
Post-pretrain数据
数据分析
数据清洗
SFT数据
什么是数据洞察
创建并查看数据洞察任务
提示词模板拼接
数据增强
数据清洗
数据集对应关系说明
数据集管理
创建数据集
导入Prefix+Suffix+Middle代码补全数据
导入Prompt+Chosen+Rejected数据
导入Prompt+Chosen或Rejected数据
导入Prompt+Image+Response数据
导入Prompt+Response数据
导入Prompt+图片数据
导入Prompt+多Response排序数据
导入Prompt集数据
导入Role(user+assistant)数据
导入纯文本数据
导出数据
管理数据集版本
模型训练
模型精调
创建训练任务
偏好对齐
CFC使用教程
RFT自定义奖励规则
RLHF
创建奖励模型训练任务
创建强化学习训练任务
创建DPO任务
创建KTO任务
创建RFT任务
创建Post-pretrain任务
创建SFT任务
查看训练列表
精调方式定义
SFT调优快速手册
什么是Post-pretrain
偏好对齐
什么是DPO训练
什么是KTO训练
什么是RFT训练
什么是RLHF训练
什么是SimPO训练
模型蒸馏
创建数据蒸馏任务
创建知识蒸馏任务
精调样板
3小时极速蒸馏 DeepSeek-R1
RFT-逻辑推理任务—“谁是好人”
UGC图片质检
作文自动点评或批改
劳动合同关键信息提取
客服对话多标签生成
小说人物角色扮演
小说人物角色扮演（KTO）
数学判题
数据飞轮-「猜你想问」
文本创作字数控制 V2
文本创作字数控制
模型精调使用技巧
什么情况下适合精调
如何从评估指标判断大模型训练效果
如何准备有价值的训练数据
模型蒸馏训练方法（情感分析任务）
英语口语练习
购物平台客服对话摘要
Prompt工程
Prompt模板
模型广场
内容安全
安全策略
干预策略
Agent开发
快速搭建应用
官方Agent
深度研究Agent
全能AI助手
模板案例
应用广场
精选应用搭建说明
企业服务
【企业服务】企业招聘智能助手
【企业服务】合同审核助手
【企业服务】图像结构化信息提取
【企业服务】智能安全巡检助手
【企业服务】港口司机服务助手
【企业服务】电力知识库助手
【企业服务】说明书解读助手
医疗健康
【医疗健康】健康问诊助手
【医疗健康】膳食金字塔老年膳食平衡AI助理
政务服务
【政务服务】上海市公积金智能助手
【政务服务】上海市医疗保险智能助手
【政务服务】上海市居住证助手
【政务服务】上海市教育政策智能助手
【政务服务】公安智能笔录审核助手
【政务服务】公文笔杆子
【政务服务】唐山市民生服务智能体
【政务服务】天津市工业和信息产业政策查询助手
【政务服务】法定退休年龄计算器
【政务服务】西安市场监管政策查询助手
【政务服务】重庆市教育政策查询助手
【政务服务】青岛市行政审批服务事项查询助手
教育培训
【教育培训】K12互动式练习题
【教育培训】K12地理小达人
【教育培训】K12练习题生成器
【教育培训】Python编程助教
【教育培训】中级安全工程师报考小助手
【教育培训】作文点评助手
【教育培训】古诗词万事通
【教育培训】图转故事小达人
【教育培训】学伴暖光
【教育培训】学校招生小助手
【教育培训】思政报告小助手
【教育培训】拍照解题
【教育培训】智课辅导小助手
【教育培训】校园问答助手
【教育培训】红灯停停停
【教育培训】翻译专家
【教育培训】职业教育试题解析助教
【教育培训】英语外教Lucas
【教育培训】论文全能助手
【教育培训】试题信息抽取
【教育培训】趣读古诗
【教育培训】鸿蒙领航者
文化旅游
【文化旅游】国家博物馆数字讲解员
【文化旅游】陕西旅游助手
新闻媒体
【新闻媒体】新闻资讯助手
智能硬件
【智能硬件】天气查询agent
【智能硬件】影视剧情搜索
【智能硬件】影视名称搜索
【智能硬件】智家超级管家
法律法规
【法律法规】民法典不在话下
社交文娱
【社交娱乐】图片标签生成
【社交文娱】傲娇华妃
【社交文娱】卡通头像制作
【社交文娱】图片诗意描述器
【社交文娱】娱乐新闻瓜瓜乐
【社交文娱】小红书文案生成助手
【社交文娱】抖音视频文案提取助手
【社交文娱】智能汽车管家
【社交文娱】毛小尖
【社交文娱】爱因斯坦
【社交文娱】遥感知识助手
营销电商
【营销电商】专业评论回复-民宿
【营销电商】中差评识别及干预
【营销电商】智能客服-会话总结
【营销电商】私人茶叶销售助手
【营销电商】长文本创作
金融证券
【金融证券】国债助手
【金融证券】复利计算器
【金融证券】熊大讲重疾险
【金融证券】银行业务经营全链助手
创建应用
自主规划Agent
概述
角色指令
声音与形象
对话
应用发布
发布配置
版本管理
应用回复
应用导入导出
应用调优
数据干预
规则干预
数据库
模型配置
知识库
组件
背景图片
记忆
文件上传说明
工作流Agent
概述
工作流节点
开始节点
结束节点
大模型节点
智能体节点
组件节点
API 节点
MCP Server节点
函数计算CFC节点
意图识别节点
全局跳转节点
分支器节点
代码节点
循环节点
参数聚合节点
知识库节点
数据库节点
记忆变量节点
文本处理节点
Query多轮改写
流式数据处理
信息收集节点
消息节点
应用配置
对话设置
记忆变量
声音与形象
背景图片
追问
应用发布
发布配置
版本管理
应用调优
规则干预
多智能体协同Agent
概述
A2A接口文档
交互式写作Agent
概述
开发态：布局介绍
开发态：基本配置
开发态：模板配置
开发态：应用发布
使用态：布局介绍
使用态：全文生成
使用态：智能编辑
使用态：导出文档
数据分析
导入其他厂商模型服务
应用分发
秒哒
模板案例
网页版
百度搜索
企业微信
微信公众号（订阅号、服务号）
微信客服
网站嵌入
创建组件
工作流创建工具及MCP
概述
创建组件
配置节点
开始节点
结束节点
大模型节点
组件节点
API 节点
MCP Server节点
函数计算CFC节点
意图识别节点
分支器节点
代码节点
循环节点
参数聚合节点
知识库节点
数据库节点
记忆变量节点
文本处理节点
Query多轮改写节点
流式数据处理
调试和发布
知识库管理
知识库简介
创建知识库
切片管理
命中测试
命中测试（召回）提供分数的计算说明和优化说明
修改知识库配置
数据库管理
专业词库管理
Prompt模板管理
卡片配置与管理
为应用回复添加卡片样式
创建我的卡片
卡片概述
卡片管理
批量任务管理
团队协作
OpenClaw
OpenClaw 使用文档
千帆 OpenClaw 接入飞书机器人教程
工具及MCP广场
工具及MCP广场
报警管理
系统管理
API KEY
配额管理
资源管理
账号权限与审计
添加消息接收人操作指南
角色与权限控制列表
账号创建与权限分配
通过子账户隔离实现账单隔离
对接云审计
API参考
模型服务
平台功能OpenAPI
介绍
API导航
通用说明
服务域名
错误码
鉴权认证
推理服务API
名词解释
推理服务API V2
Agent开发
通用说明
认证鉴权
Agent
工具
知识库
错误码
官方工具及MCP
百度百科
秒懂百科
百度百科
百科词条
百科星图
热搜榜单
百度热搜
营销电商
智能商品营销文案生成
商品生动化标题生成
智能商品搜索
实用工具
百度网盘AI视频笔记
百度文库智能PPT（Agent版）
百度文库AI绘本
百度网盘AI纪要
百度文库智能PPT
百度文库智能PPT大纲
百度文库智能PPT
获取PPT模版
自定义模版上传
百度文库AI长文
百度文库AI长文大纲
百度文库AI长文
百度文库智能研报
百度文库智能研报大纲
百度文库智能研报
智能Query改写
广告素材质量审核
智能识图
百度AI搜索
百度搜索
智能搜索生成
相似图搜索
错误码
服务可用性协议
产品简介-百度AI助手与智能搜索生成
SDK参考
模型服务
推理服务API
OpenAI SDK兼容介绍
应用开发
SDK功能更新记录
SDK介绍
快速入门
安装说明
版本说明
应用
基础能力组件列表
应用信息
应用调用
获取模型列表
工具及MCP
工作流工具调用
知识库管理
知识库
声音&形象
数字人应用调用COOKBOOK
语音对话COOKBOOK
常见问题
AppBuilder SDK 运行环境超参配置说明
Appbuilder Trace跟踪功能基本用法
Phoneix可视化软件的进阶用法
UserSession类
二次开发
公有云部署
常见问题
模型服务
API&SDK相关
产品使用相关
产品计费相关
应用开发
相关协议
百度千帆协议概述
服务等级协议SLA
大模型技术体验服务协议
生成式人工智能服务备案信息参考
百度千帆大模型服务及Agent开发平台用户协议
百度千帆大模型服务及Agent开发平台专项约定
内容安全策略自定义功能服务协议
所有文档
没有找到结果，请重新输入
百度千帆·大模型服务及Agent开发平台
平台介绍
平台简介
更新动态
产品公告
模型服务
模型更新记录
平台更新记录
模型版本升级及退役机制
Token Plan个人版上线及Coding Plan停售公告
Coding Plan 产品升级公告
批量推理促销活动说明
Anthropic API 返回参数input_tokens含义更新
7月28日服务停服通知
模型服务 V1 接口下线及迁移说明
应用开发
模型赠送资源下线公告
Agent模型服务升级及切换公告
知识库高级解析服务计费公告
共享资源知识库检索计费公告
工具及MCP广场
官方工具下架公告
工具限时体验额度到期通知
教育类工具下架公告
热榜类工具下架公告
模型介绍
快速开始-模型服务调用
模型列表
功能特性
模型能力
模型默认参数说明
Token Plan 个人版
Token Plan 个人版
快速开始
接入AI工具
OpenClaw
Claude Code
OpenCode
Cursor
CodeX
Qwen Code
Kilo CLI
Kilo Code
Cline
Cherry Studio
CC-Switch
Hermes Agent
DeepSeek Harness
ZCode
Coding Plan 升级与权益调整说明
Coding Plan 迁移至 Token Plan 用户授权协议
使用须知
Token Plan 企业版
Token Plan 企业版
快速开始
团队管理
接入工具
OpenCode
CC-Switch
Qwen Code
Kilo CLI
Kilo Code
Cherry Studio
Cline
CodeX
Cursor
Claude Code
OpenClaw
Zcode
Hermes Agent
DeepSeek Harness
Coding Plan
Coding Plan
接入AI工具
OpenClaw
Claude Code
OpenCode
Cursor
Cline
CodeX
Qwen Code Companion
Cherry studio
Kilo CLI
Kilo Code
cc-switch
计费说明
模型服务
模型服务计费
价格详情
数据管理
模型精调
模型蒸馏
模型评估
续费和退款说明
退款申请说明
续费和自动续费
token计算说明
模型token计算说明
通过cfc发布token计算在线API
新用户免费额度
成本拆分
自定义接入点（Endpoint）成本拆分迁移指南
成本拆分使用说明
官方工具
百度AI搜索
智能搜索生成
智能搜索生成高性能版
百度搜索
相似图搜索
API Key维度分账
QPS叠加包
应用开发
Agent开发平台
官方Agent
深度研究Agent
AI助手
Token 福利包
Token 福利包
接入工具
Claude Code
OpenClaw
OpenCode
Cursor
CodeX
Cline
Qwen Code
Cherry Studio
Kilo Code
Kilo CLI
CC-Switch
知识库
最佳实践
模型服务
模型管理
HuggingFace模型下载、部署、调用实战
模型训练
文生文SFT最佳实践
图像理解SFT最佳实践
Post-pretrain最佳实践
生态集成
通过服务网卡私网访问千帆模型服务
批量推理
Files + Batch 接口使用指南（OpenAI 兼容版）
应用开发
官方模板介绍
数字人应用：创建、调试和SDK集成
数字人应用：美食顾问
记忆变量：MBTI性格测试助手
自主规划Agent最佳实践
用户旅程地图生成
自主规划Agent：开发AI手机实战，让手机开启「自动驾驶」
自主规划Agent：营销策略设计师
自主规划Agent一键发布到秒哒：茶叶销售线上转型实践
工作流Agent最佳实践
工作流Agent：“金牌销售”车险续保Agent
工作流Agent：如何使用全局跳转节点搭建飞行客服小助手
工作流Agent：机票订单处理助手
工作流大模型节点高级配置使用案例
多智能体协同Agent最佳实践
用“多智能体协同Agent”一分钟搭建专属研究Agent，帮企业做深度研究
自定义工具最佳实践
工作流组件： API的增删查
工作流组件：如何用知识库节点和API节点搭建个人减脂小助手
工作流组件：深度使用实践
组件工作流：使用记忆变量节点构建个人化导购小助手
知识库最佳实践
图谱增强RAG：医疗诊断助手
多模态RAG：图文问答助手
对象存储（BOS）与向量检索(BES)：企业级知识库创建
检索增强生成（RAG）应用：提示词调试
航司行李规格查询助手
表格型知识库与数据库：课程信息检索
卡片功能最佳实践
智能菜谱推荐应用搭建
官方工具
百度AI搜索
百度AI搜索应用搭建
百度搜索组件：一文读懂多种搜索组合方式
如何在OpenClaw调用官方工具
OpenClaw最佳实践
冬奥智能解说助手
我的职场PPT生成助手—热点追踪
孩子成长记录绘本助手
平台操作
模型服务
快速开始-平台功能使用
体验中心
模型服务
在线服务
批量推理
推理日志投递与解绑
推理服务监控告警
自定义接入点
模型管理
我的模型
HuggingFace格式模型导入
将HuggingFace模型下载到BOS
模型创建与管理
模型评估
效果评估
产品描述
自动评估
创建自动评估任务
查看与管理自动评估任务
人工评估
创建人工评估任务
查看与管理人工评估任务
性能评估
模型压缩
数据管理
推理结果集
使用推理结果集
创建推理结果集
创建推理结果集任务
通用数据集
推理日志挖掘
数据标注
众测标注
在线标注
多人标注
数据洞察与处理
Post-pretrain数据
数据分析
数据清洗
SFT数据
什么是数据洞察
创建并查看数据洞察任务
提示词模板拼接
数据增强
数据清洗
数据集对应关系说明
数据集管理
创建数据集
导入Prefix+Suffix+Middle代码补全数据
导入Prompt+Chosen+Rejected数据
导入Prompt+Chosen或Rejected数据
导入Prompt+Image+Response数据
导入Prompt+Response数据
导入Prompt+图片数据
导入Prompt+多Response排序数据
导入Prompt集数据
导入Role(user+assistant)数据
导入纯文本数据
导出数据
管理数据集版本
模型训练
模型精调
创建训练任务
偏好对齐
CFC使用教程
RFT自定义奖励规则
RLHF
创建奖励模型训练任务
创建强化学习训练任务
创建DPO任务
创建KTO任务
创建RFT任务
创建Post-pretrain任务
创建SFT任务
查看训练列表
精调方式定义
SFT调优快速手册
什么是Post-pretrain
偏好对齐
什么是DPO训练
什么是KTO训练
什么是RFT训练
什么是RLHF训练
什么是SimPO训练
模型蒸馏
创建数据蒸馏任务
创建知识蒸馏任务
精调样板
3小时极速蒸馏 DeepSeek-R1
RFT-逻辑推理任务—“谁是好人”
UGC图片质检
作文自动点评或批改
劳动合同关键信息提取
客服对话多标签生成
小说人物角色扮演
小说人物角色扮演（KTO）
数学判题
数据飞轮-「猜你想问」
文本创作字数控制 V2
文本创作字数控制
模型精调使用技巧
什么情况下适合精调
如何从评估指标判断大模型训练效果
如何准备有价值的训练数据
模型蒸馏训练方法（情感分析任务）
英语口语练习
购物平台客服对话摘要
Prompt工程
Prompt模板
模型广场
内容安全
安全策略
干预策略
Agent开发
快速搭建应用
官方Agent
深度研究Agent
全能AI助手
模板案例
应用广场
精选应用搭建说明
企业服务
【企业服务】企业招聘智能助手
【企业服务】合同审核助手
【企业服务】图像结构化信息提取
【企业服务】智能安全巡检助手
【企业服务】港口司机服务助手
【企业服务】电力知识库助手
【企业服务】说明书解读助手
医疗健康
【医疗健康】健康问诊助手
【医疗健康】膳食金字塔老年膳食平衡AI助理
政务服务
【政务服务】上海市公积金智能助手
【政务服务】上海市医疗保险智能助手
【政务服务】上海市居住证助手
【政务服务】上海市教育政策智能助手
【政务服务】公安智能笔录审核助手
【政务服务】公文笔杆子
【政务服务】唐山市民生服务智能体
【政务服务】天津市工业和信息产业政策查询助手
【政务服务】法定退休年龄计算器
【政务服务】西安市场监管政策查询助手
【政务服务】重庆市教育政策查询助手
【政务服务】青岛市行政审批服务事项查询助手
教育培训
【教育培训】K12互动式练习题
【教育培训】K12地理小达人
【教育培训】K12练习题生成器
【教育培训】Python编程助教
【教育培训】中级安全工程师报考小助手
【教育培训】作文点评助手
【教育培训】古诗词万事通
【教育培训】图转故事小达人
【教育培训】学伴暖光
【教育培训】学校招生小助手
【教育培训】思政报告小助手
【教育培训】拍照解题
【教育培训】智课辅导小助手
【教育培训】校园问答助手
【教育培训】红灯停停停
【教育培训】翻译专家
【教育培训】职业教育试题解析助教
【教育培训】英语外教Lucas
【教育培训】论文全能助手
【教育培训】试题信息抽取
【教育培训】趣读古诗
【教育培训】鸿蒙领航者
文化旅游
【文化旅游】国家博物馆数字讲解员
【文化旅游】陕西旅游助手
新闻媒体
【新闻媒体】新闻资讯助手
智能硬件
【智能硬件】天气查询agent
【智能硬件】影视剧情搜索
【智能硬件】影视名称搜索
【智能硬件】智家超级管家
法律法规
【法律法规】民法典不在话下
社交文娱
【社交娱乐】图片标签生成
【社交文娱】傲娇华妃
【社交文娱】卡通头像制作
【社交文娱】图片诗意描述器
【社交文娱】娱乐新闻瓜瓜乐
【社交文娱】小红书文案生成助手
【社交文娱】抖音视频文案提取助手
【社交文娱】智能汽车管家
【社交文娱】毛小尖
【社交文娱】爱因斯坦
【社交文娱】遥感知识助手
营销电商
【营销电商】专业评论回复-民宿
【营销电商】中差评识别及干预
【营销电商】智能客服-会话总结
【营销电商】私人茶叶销售助手
【营销电商】长文本创作
金融证券
【金融证券】国债助手
【金融证券】复利计算器
【金融证券】熊大讲重疾险
【金融证券】银行业务经营全链助手
创建应用
自主规划Agent
概述
角色指令
声音与形象
对话
应用发布
发布配置
版本管理
应用回复
应用导入导出
应用调优
数据干预
规则干预
数据库
模型配置
知识库
组件
背景图片
记忆
文件上传说明
工作流Agent
概述
工作流节点
开始节点
结束节点
大模型节点
智能体节点
组件节点
API 节点
MCP Server节点
函数计算CFC节点
意图识别节点
全局跳转节点
分支器节点
代码节点
循环节点
参数聚合节点
知识库节点
数据库节点
记忆变量节点
文本处理节点
Query多轮改写
流式数据处理
信息收集节点
消息节点
应用配置
对话设置
记忆变量
声音与形象
背景图片
追问
应用发布
发布配置
版本管理
应用调优
规则干预
多智能体协同Agent
概述
A2A接口文档
交互式写作Agent
概述
开发态：布局介绍
开发态：基本配置
开发态：模板配置
开发态：应用发布
使用态：布局介绍
使用态：全文生成
使用态：智能编辑
使用态：导出文档
数据分析
导入其他厂商模型服务
应用分发
秒哒
模板案例
网页版
百度搜索
企业微信
微信公众号（订阅号、服务号）
微信客服
网站嵌入
创建组件
工作流创建工具及MCP
概述
创建组件
配置节点
开始节点
结束节点
大模型节点
组件节点
API 节点
MCP Server节点
函数计算CFC节点
意图识别节点
分支器节点
代码节点
循环节点
参数聚合节点
知识库节点
数据库节点
记忆变量节点
文本处理节点
Query多轮改写节点
流式数据处理
调试和发布
知识库管理
知识库简介
创建知识库
切片管理
命中测试
命中测试（召回）提供分数的计算说明和优化说明
修改知识库配置
数据库管理
专业词库管理
Prompt模板管理
卡片配置与管理
为应用回复添加卡片样式
创建我的卡片
卡片概述
卡片管理
批量任务管理
团队协作
OpenClaw
OpenClaw 使用文档
千帆 OpenClaw 接入飞书机器人教程
工具及MCP广场
工具及MCP广场
报警管理
系统管理
API KEY
配额管理
资源管理
账号权限与审计
添加消息接收人操作指南
角色与权限控制列表
账号创建与权限分配
通过子账户隔离实现账单隔离
对接云审计
API参考
模型服务
平台功能OpenAPI
介绍
API导航
通用说明
服务域名
错误码
鉴权认证
推理服务API
名词解释
推理服务API V2
Agent开发
通用说明
认证鉴权
Agent
工具
知识库
错误码
官方工具及MCP
百度百科
秒懂百科
百度百科
百科词条
百科星图
热搜榜单
百度热搜
营销电商
智能商品营销文案生成
商品生动化标题生成
智能商品搜索
实用工具
百度网盘AI视频笔记
百度文库智能PPT（Agent版）
百度文库AI绘本
百度网盘AI纪要
百度文库智能PPT
百度文库智能PPT大纲
百度文库智能PPT
获取PPT模版
自定义模版上传
百度文库AI长文
百度文库AI长文大纲
百度文库AI长文
百度文库智能研报
百度文库智能研报大纲
百度文库智能研报
智能Query改写
广告素材质量审核
智能识图
百度AI搜索
百度搜索
智能搜索生成
相似图搜索
错误码
服务可用性协议
产品简介-百度AI助手与智能搜索生成
SDK参考
模型服务
推理服务API
OpenAI SDK兼容介绍
应用开发
SDK功能更新记录
SDK介绍
快速入门
安装说明
版本说明
应用
基础能力组件列表
应用信息
应用调用
获取模型列表
工具及MCP
工作流工具调用
知识库管理
知识库
声音&形象
数字人应用调用COOKBOOK
语音对话COOKBOOK
常见问题
AppBuilder SDK 运行环境超参配置说明
Appbuilder Trace跟踪功能基本用法
Phoneix可视化软件的进阶用法
UserSession类
二次开发
公有云部署
常见问题
模型服务
API&SDK相关
产品使用相关
产品计费相关
应用开发
相关协议
百度千帆协议概述
服务等级协议SLA
大模型技术体验服务协议
生成式人工智能服务备案信息参考
百度千帆大模型服务及Agent开发平台用户协议
百度千帆大模型服务及Agent开发平台专项约定
内容安全策略自定义功能服务协议
文档中心
百度千帆·大模型服务及Agent开发平台
模型介绍
模型列表
文档中心
百度千帆·大模型服务及Agent开发平台
模型介绍
模型列表
...
模型列表
更新时间：2026-09-07
推荐模型
|
| 旗舰模型
| ERNIE-5.0
| ERNIE-5.1
| ERNIE-4.5-Turbo-128K
| DeepSeek-V4-Pro
| 使用场景
| 文心新一代模型文心5.0是原生全模态大模型，采用原生的全模态统一建模技术，将文本、图像、音频、视频联合建模，具备综合的全模态能力。文心5.0基础能力全面升级，基准测试集表现出色，多模态理解、指令遵循、创意写作、事实性、智能体规划与工具应用等表现尤其出色。
| ERNIE 5.1是文心系列最新模型，基础能力全面升级，在智能体、知识、推理、深度搜索等方面均有显著提升。此次升级发布，采用分离式全异步强化学习技术，针对性解决大模型向智能体自主决策演进中面临的训推数值偏差、异构资源利用率低及长尾效应导致的全局等挑战。配套采用规模化智能体后训练技术来提升模型能力与泛化表现，让环境、专家、融合三步协同 ，既保证了训练效率，也显著提升模型在复杂任务中的稳定性与表现。
| 核心定位：更好的满足多轮长历史对话处理、长文档理解问答任务。
适用场景：
1)复杂语义理解：支持中文知识问答、文学创作，尤其擅长文档理解（如DocVQA任务）。
2)数学推理：在中文数学问题（CMath基准）表现突出。
| 核心定位：超强通用旗舰大模型，Agent能力大幅提高。
适用场景：复杂 Agent 智能体任务：多工具连环调用、多步骤业务自主规划与闭环执行；长文本深度处理：合同卷宗、金融研报、整站代码库、长篇文献一次性通读解析；高阶科研与工程场景：高难度数理推理、代码全工程开发、学术论证推演、企业级复杂业务逻辑建模。
| 上下文长度
（Token数）
| 128k
| 128k
| 128k
| 1M
| 最大输出长度
（Token数）
| 64k
| 64k
| 16k
| 384k
文本生成
教程：文本生成 | API：文本生成
ERNIE系列-旗舰模型
|
| 模型名称
| model参数
接入点ID
| 上下文长度
(token)
| 最大输入
(token)
| 最大输出
(token)
| 默认流控
| ERNIE 5.1
| ernie-5.1
| 128k
| 119k
| [1，65536]
| RPM = 60
TPM = 150000
| ERNIE 5.0
| ernie-5.0
| 128k
| 119k
| [1，65536]
| RPM = 60
TPM = 150000
| ERNIE 5.0
| ernie-5.0-thinking-preview
| 128k
| 119k
| [1，65536]
| RPM = 60
TPM = 150000
| ERNIE 5.0
| ernie-5.0-thinking-latest
| 128k
| 119k
| [1，65536]
| RPM = 60
TPM = 150000
| ERNIE 5.0
| ernie-5.0-thinking-exp
| 128k
| 119k
| [1，65536]
| RPM = 6
TPM = 32000
| ERNIE 4.5 Turbo
| ernie-4.5-turbo-32k
| 32k
| 27k
| [2，12288]
| RPM = 5000
TPM = 400000
| ERNIE 4.5 Turbo
| ernie-4.5-turbo-128k
| 128k
| 123k
| [2，12288]
| RPM = 5000
TPM = 400000
| ERNIE 4.5 Turbo
| ernie-4.5-turbo-20260402
| 128k
| 123k
| [2，12288]
| RPM = 60
TPM = 150000
| ERNIE 4.5 Turbo VL
| ernie-4.5-turbo-vl
| 128k
| 123k
| [2，16384]
| RPM = 1000
TPM = 200000
| ERNIE 4.5 Turbo VL
| ernie-4.5-turbo-vl-32k
| 32k
| 27k
| [2，12288]
| RPM = 1000
TPM = 200000
DeepSeek系列
|
| 模型名称
| 版本
| model参数
接入点ID
| 上下文长度
(token)
| 最大输入
(token)
| 最大输出
(token)
| 默认流控
| DeepSeek-V4
| DeepSeek-V4-Pro-0813
| deepseek-v4-pro-0813
| 1M
| 1M
| [1,393216]
| RPM = 60
TPM = 1000000
| DeepSeek-V4
| DeepSeek-V4-Pro
| deepseek-v4-pro
| 1M
| 1M
| [1,393216]
| RPM = 60
TPM = 1000000
| DeepSeek-V4
| DeepSeek-V4-Flash-0731
| deepseek-v4-flash-0731
| 1M
| 1M
| [1,393216]
| RPM = 60
TPM = 250000
| DeepSeek-V4
| DeepSeek-V4-Flash(即将下线）
| deepseek-v4-flash
| 1M
| 1M
| [1,393216]
| RPM = 60
TPM = 1000000
| DeepSeek-V3.2
| DeepSeek-V3.2(即将下线）
| deepseek-v3.2
| 128k
| 96k
| 32k
默认4k
| RPM = 5000
TPM = 1000000
DeepSeek-V3.2 当前对应DeepSeek-V3.2 正式版，model参数/接入点ID不变。
视觉理解
教程：视觉理解 | API：视觉理解
ERNIE系列
|
| 模型名称
| model参数
接入点ID
| 上下文长度
(token)
| 最大输入
(token)
| 最大输出
(token)
| 默认流控
| ERNIE 5.0
| ernie-5.0
| 128k
| 119k
| [1，65536]
| RPM = 60
TPM = 150000
| ERNIE 5.0
| ernie-5.0-thinking-preview
| 128k
| 119k
| [1，65536]
| RPM = 60
TPM = 150000
| ERNIE 5.0
| ernie-5.0-thinking-latest
| 128k
| 119k
| [1，65536]
| RPM = 60
TPM = 150000
| ERNIE 4.5 Turbo VL
| ernie-4.5-turbo-vl
| 128k
| 123k
| [2，16384]
| RPM = 1000
TPM = 200000
| ERNIE 4.5 Turbo VL
| ernie-4.5-turbo-vl-32k
| 32k
| 27k
| [2，12288]
| RPM = 1000
TPM = 200000
InternVL系列
|
| 模型名称
| 版本
| model参数
接入点ID
| 上下文长度
(token)
| 最大输入
(token)
| 最大输出
(token)
| 默认流控
| InternVL3
| InternVL3-38B
| internvl3-38b
| 32k
| 24k
| 8k
默认2k
| RPM = 60
TPM = 60000
深度思考
教程：深度思考 | API：文本生成
注：深度思考模型在输出最终回答之前会先输出一段思维链内容，以下最大输出长度为包含思维链输出的总长度。
ERNIE系列
|
| 模型名称
| 版本
| model参数
接入点ID
| 上下文长度
(token)
| 最大输入
(token)
| 最大输出
(token)
| 思维链长度
(token)
| 默认流控
| ERNIE 5.0
| ERNIE-5.0
| ernie-5.0
| 128k
| 119k
| [1，65536]
| 60k
| RPM = 60
TPM = 150000
| ERNIE 5.0
| ERNIE-5.0-Thinking-Preview
| ernie-5.0-thinking-preview
| 128k
| 119k
| [1，65536]
| 60k
| RPM = 60
TPM = 150000
| ERNIE 5.0
| ERNIE-5.0-Thinking-Latest
| ernie-5.0-thinking-latest
| 128k
| 119k
| [1，65536]
| 60k
| RPM = 60
TPM = 150000
| ERNIE X1.1
| ERNIE-X1.1-Preview（即将下线）
| ernie-x1.1-preview
| 64k
| 55k
| [1，65536]
| 64k
| RPM = 60
TPM = 60000
| ERNIE X1.1
| ERNIE-X1.1（即将下线）
| ernie-x1.1
| 64k
| 55k
| [1，65536]
| 64k
| RPM = 300
TPM = 300000
ernie系列思考模型，不支持thinking_budget。同时，max_tokens限制reasoning_content+content总长度。
DeepSeek满血版
|
| 模型名称
| 版本
| model参数
接入点ID
| 上下文长度
(token)
| 最大输入
(token)
| 最大输出
(token)
| 思维链长度
(token)
| 默认流控
| DeepSeek-V4
| DeepSeek-V4-Pro-0813
| deepseek-v4-pro-0813
| 1M
| 1M
| [1,393216]
| 32k
| RPM = 60
TPM = 1000000
| DeepSeek-V4
| DeepSeek-V4-Pro
| deepseek-v4-pro
| 1M
| 1M
| [1,393216]
| 32k
| RPM = 60
TPM = 1000000
| DeepSeek-V4
| DeepSeek-V4-Flash-0731
| deepseek-v4-flash-0731
| 1M
| 1M
| [1,393216]
| 32k
| RPM = 60
TPM = 250000
| DeepSeek-V4
| DeepSeek-V4-Flash（即将下线）
| deepseek-v4-flash
| 1M
| 1M
| [1,393216]
| 32k
| RPM = 60
TPM = 1000000
| DeepSeek-V3.2-Think
| DeepSeek-V3.2-Think(即将下线）
| deepseek-v3.2-think
| 144k
| 96k
| [1,65536]
| 32k
| RPM = 60
TPM = 150000
DeepSeek-V3.2-Think 当前对应DeepSeek-V3.2 正式版，model参数/接入点ID不变。
DeepSeek系列思考模型支持设置thinking_budget，输出长度控制请参考上下文管理。
Qwen系列
|
| 模型名称
| 版本
| model参数
接入点ID
| 上下文长度
(token)
| 最大输入
(token)
| 最大输出
(token)
| 思维链长度
(token)
| 默认流控
| Qwen3.5
| Qwen3.5-397B-A17B
| qwen3.5-397b-a17b
| 256k
| 252k
| [1，65536]
| 80k
| RPM = 60
TPM = 250000
| Qwen3.5
| Qwen3.5-122B-A10B
| qwen3.5-122b-a10b
| 256k
| 252k
| [1，65536]
| 80k
| RPM = 60
TPM = 250000
| Qwen3.5
| Qwen3.5-27B
| qwen3.5-27b
| 256k
| 252k
| [1，65536]
| 80k
| RPM = 60
TPM = 250000
| Qwen3.5
| Qwen3.5-35B-A3B
| qwen3.5-35b-a3b
| 256k
| 252k
| [1，65536]
| 80k
| RPM = 60
TPM = 250000
其他
|
| 模型名称
| 版本
| model参数
接入点ID
| 上下文长度
(token)
| 最大输入
(token)
| 最大输出
(token)
| 默认流控
| GLM-5.3-Flash
| GLM-5.3-Flash
| glm-5.3-flash
| 1M
| 1M
| [1，131072]
| RPM = 60
TPM = 1000000
| GLM-5.3
| GLM-5.3
| glm-5.3
| 1M
| 1M
| [1，131072]
| RPM = 60
TPM = 1000000
| GLM-5.2
| GLM-5.2
| glm-5.2
| 1M
| 1M
| [1，131072]
| RPM = 60
TPM = 1000000
| GLM-5.1
| GLM-5.1
| glm-5.1
| 198k
| 198k
| [1，131072]
| RPM = 60
TPM = 250000
| GLM-5
| GLM-5（即将下线）
| glm-5
| 198k
| 198k
| [1，131072]
| RPM = 60
TPM = 250000
| Kimi-K2.6
| Kimi-K2.6（即将下线）
| kimi-k2.6
| 256k
| 256k
| [1，262144]
| RPM = 60
TPM = 250000
OCR
|
| 模型名称
| 版本
| model参数
接入点ID
| 上下文长度
(token)
| 最大输入
(token)
| 最大输出
(token)
| 默认流控
| PaddleOCR-VL
| PaddleOCR-VL-0.9B
| paddleocr-vl-0.9b
| -
| -
| -
| RPM = 60
| PP-StructureV3
| PP-StructureV3
| pp-structurev3
| -
| -
| -
| RPM = 60
| DeepSeek-OCR
| DeepSeek-OCR
| deepseek-ocr
| 8k
| 8k
| 8k
默认4k
| RPM = 60
TPM = 60000
| Qianfan-OCR
| Qianfan-OCR
| qianfan-ocr
| 32k
| 32k
| 28k
| RPM = 60
TPM = 60000
图像生成
教程：图像生成 | API：图片生成
|
| 模型名称
| 版本
| model参数
接入点ID
| 最大输入(字符)
| 默认流控
| MuseSteamer Air Image
| MuseSteamer-Air-Image
| musesteamer-air-image
| 1000字符
| 6RPM
| Qwen Image
| Qwen-Image
| qwen-image
| 800字符
| 6RPM
图像编辑
教程：图像编辑 | API：图片编辑
|
| 模型名称
| 版本
| model参数
接入点ID
| 最大输入(字符)
| 默认流控
| Qwen Image Edit
| Qwen-Image-Edit
| qwen-image-edit
| 800字符
| 6RPM
文本向量
教程：向量 | API：向量
|
| 模型名称
| 版本
| model参数
接入点ID
| 向量维度
| 最大输入(文本数量)
| 每个文本上下文长度
(token)
| Embedding-V1
| Embedding-V1
| embedding-v1
| 384
| 16
| 384
| tao-8k
| tao-8k
| tao-8k
| 1024
| 1
| 8192
| bge-large-zh
| bge-large-zh
| bge-large-zh
| 1024
| 16
| 512
| bge-large-en
| bge-large-en
| bge-large-en
| 1024
| 16
| 512
| Qwen3-Embedding-0.6B
| Qwen3-Embedding-0.6B
| qwen3-embedding-0.6b
| 1024
| 16
| 8192
| Qwen3-Embedding-4B
| Qwen3-Embedding-4B
| qwen3-embedding-4b
| 2560
| 16
| 8192
| Qwen3-Embedding-8B
| Qwen3-Embedding-8B
| qwen3-embedding-8b
| 4096
| 16
| 8192
重排序
教程：重排序 | API：重排序
|
| 模型名称
| 版本
| model参数
接入点ID
| 最大输入(字符)
| 默认流控
| bce-reranker-base
| bce-reranker-base
| bce-reranker-base
| query：400 tokens/1600字符
document：1K tokens/4000字符
| RPM = 1800
TPM = 800000
| Qwen3-Reranker-0.6B
| Qwen3-Reranker-0.6B
| qwen3-reranker-0.6b
| 输入最大32k
query：1条
| RPM = 1800
TPM = 800000
| Qwen3-Reranker-4B
| Qwen3-Reranker-4B
| qwen3-reranker-4b
| 输入最大32k
query：1条
| RPM = 1800
TPM = 800000
| Qwen3-Reranker-8B
| Qwen3-Reranker-8B
| qwen3-reranker-8b
| 输入最大32k
query：1条
| RPM = 1800
TPM = 800000
评价此篇文章
有帮助没帮助
上一篇
快速开始-模型服务调用
下一篇
功能特性
文档助手
文档反馈