...
test('T02 maas_get_changes 真实数据 + 文本/结构一致', async () => {
    ...
    assert.ok(sc.datasetVersion.startsWith('ds_'));
    const items = sc.items;
    assert.ok(items.length > 0 && items.length <= 10);
    // 文本含版本（前 8 hex）、数据截至、条目 id
    const text = r.content[0].text;
    assert.ok(text.includes(sc.datasetVersion.slice(3, 11)), '文本含版本片段');
    assert.ok(text.includes(String(sc.dataThrough)), '文本含 dataThrough');
    assert.ok(text.includes(items[0].id.slice(0, 12)), '文本含条目 id');
});
test('T02 maas_get_prices 真实数据（金额/币种/单位/证据）', async () => {
    ...
    for (const p of items) {
        ...
        assert.ok(['USD', 'CNY'].includes(p.currency));
...
test('T02 maas_get_item / evidence / weekly 真实链路（含文本元数据，复验 P1-2）', async () => {
    ...
    assert.ok((ev.structuredContent.item.excerptText).length > 0);
    const wk = await call('maas_get_weekly');
    assert.ok(!wk.isError);
    const wItem = wk.structuredContent.item;
...
test('T02b P1-2 第二轮：五工具及 weekly 三种模式逐一含版本/截至/覆盖', async () => {
    ...
    const evId = (await call('maas_get_item', { id }))
        .structuredContent.item.evidenceIds[0];
    const cases = [
        ['maas_get_changes', {}],
...
test('T03 changes（同条件）REST ≡ MCP', async () => {
    ...
    for (const [qs, args] of cases) {
        const rest = await (await fetch(`${restBase}/changes?${qs}`)).json();
        const mcp = await call('maas_get_changes', args);
        assert.ok(!mcp.isError, `MCP 失败: ${JSON.stringify(mcp.structuredContent?.error)}`);
        assert.deepEqual(norm(mcp.structuredContent), norm(rest), `changes ${qs} REST/MCP 不一致`);
    }
});
test('T03 prices（model 精确 + q 包含）REST ≡ MCP', async () => {
    // 取真实 model 名
    const sample = await call('maas_get_prices', { limit: 1 });
    const modelKey = sample.structuredContent.items[0].modelKey;
    const rest = await (await fetch(`${restBase}/prices?model=${encodeURIComponent(modelKey)}&limit=10`)).json();
    ...
    const restQ = await (await fetch(`${restBase}/prices?q=${encodeURIComponent(frag)}&limit=10`)).json();
...
test('T03 weekly REST ≡ MCP', async () => {
    const rest = await (await fetch(`${restBase}/weekly?limit=3`)).json();
...
test('T03 翻页一致性（MCP 翻页与 REST 全量）', async () => {
    ...
    const rest = await (await fetch(`${restBase}/changes?limit=100`)).json();
    const restIds = rest.items.map((x) => x.id);
    ...
    while (guard++ < 10) {
        ...
        mcpIds.push(...sc.items.map((x) => x.id));
        cursor = sc.page.nextCursor ?? undefined;
        if (!cursor)
            break;
    }
    // REST 100 条与 MCP 前若干页（30 的整数倍窗口内）比较：MCP 页数 ≥ 3 时
    // 前 90 条应与 REST 前 90 条一致
    assert.equal(mcpIds.length >= 90 || mcpIds.length === restIds.length, true, `MCP 总数 ${mcpIds.length} vs REST ${restIds.length}`);
