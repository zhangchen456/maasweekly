...
export class ReleaseFixture {
    ...
    writeRelease(version, opts = {}) {
        ...
        changes.sort((x, y) => y.observationDate.localeCompare(x.observationDate) || x.id.localeCompare(y.id));
        ...
        const evidenceIds = new Set(changes.flatMap((c) => c.evidenceIds ?? []));
        ...
        const retained = oldManifest
            ...
            : [];
        const prevRetained = retained.slice(0, 1).map((m) => ({
