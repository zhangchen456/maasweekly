...
test('T04 changes 默认 limit=10 + 截断 + 翻页无重复遗漏', async () => {
    ...
    assert.equal(sc.items.length, 10);
    assert.equal(sc.page.limit, 10);
    const next = sc.page.nextCursor;
    ...
    while (cursor && guard++ < 20) {
        ...
        seen.push(...psc.items.map((x) => x.id));
        cursor = psc.page.nextCursor;
    }
...
test('T06 item→evidence 闭环 + 恶意摘录原样数据', async () => {
    ...
    assert.ok(item.structuredContent.item.revisionHistory);
    const evIds = item.structuredContent.item.evidenceIds;
    assert.equal(evIds.length, 1);
    const ev = await call('maas_get_evidence', { id: evIds[0] });
    assert.ok(!ev.isError);
    const evItem = ev.structuredContent.item;
    ...
    const wdItem = wd.structuredContent.items
    ...
    assert.equal(wdDetail.structuredContent.item.status, 'withdrawn');
});
// ---------------------------------------------------------------------------
// T07：weekly
// ---------------------------------------------------------------------------
test('T07 weekly 最新/指定/列表/id-limit 互斥', async () => {
    ...
    const item = latest.structuredContent.item;
    ...
    assert.equal(spec.structuredContent.item.id, '2026-08-25');
    const list = await call('maas_get_weekly', { limit: 1 });
    const items = list.structuredContent.items;
...
test('T08b 无有效数据 → no_data_available（不用模型知识补答）', async () => {
    ...
    await c.connect(new StreamableHTTPClientTransport(new URL(`http://127.0.0.1:${srv.address().port}/api/mcp`)));
    const r = await c.callTool({ name: 'maas_get_changes', arguments: {} });
    assert.ok(r.isError);
...
test('T10 Origin 缺省放行 / 白名单命中 / 拒绝无副作用', async () => {
    ...
    // 白名单 Origin
    const r1 = await rawPost(JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/list' }), { Origin: 'https://daily.maas.click' });
    assert.equal(r1.status, 200);
    // 非白名单 → 403
    const r2 = await rawPost(JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/list' }), { Origin: 'https://evil.example' });
...
test('T11 新版本切换 + cursor 旧版本 + 损坏旧版拒载', async () => {
    ...
    const cursor = p1.structuredContent.page.nextCursor;
    // 发布 V3 并 reload
    ...
    assert.equal(p2.structuredContent.datasetVersion.slice(0, 5), 'ds_33');
    // 旧 cursor 继续读 V2
    const p3 = await call('maas_get_changes', { cursor });
    assert.ok(!p3.isError);
    assert.equal(p3.structuredContent.datasetVersion, V2);
    ...
    await c2.connect(new StreamableHTTPClientTransport(new URL(`http://127.0.0.1:${srv2.address().port}/api/mcp`)));
    // 再损坏（此时两个 holder 都可能缓存；直接用从未加载过 V2 的新 holder 场景已覆盖缓存外路径——
    // 简化：篡改后新 holder 的 getOrLoad 返回 null）
    writeFileSync(p, orig.replace('1.000000', '999.000000'));
    try {
        const r = await c2.callTool({ name: 'maas_get_prices', arguments: { cursor: 'x' } });
