...
import { MCP_LIMITS, getItem, getEvidence, getWeekly, runListQuery, } from './query.js';
import { PUBLIC_BASE_URL } from './public-consts.js';
// ---------------------------------------------------------------------------
// 工具参数 schema（zod raw shape；入参名与 REST 查询参数完全同名）
// ---------------------------------------------------------------------------
const providerField = z.string().optional().describe('平台 ID（providerId），如 openai、alibaba。有效值以 status.providers 为准；未知值返回错误而非空列表');
const qField = z.string().optional().describe('文本包含匹配（trim + 大小写不敏感）。查价格时按模型名包含匹配；与 model 的区别：model 是精确匹配，q 是包含匹配。不确定完整模型名时用 q');
const limitField = z.number().int().min(1).max(30).optional().describe('本页条数，默认 10，最大 30。结果被截断时会返回 nextCursor，只传 cursor 获取下一页');
const cursorField = z.string().max(4096).optional().describe('上一页返回的 nextCursor，原样传入且只能单独传（与其他筛选参数同时传会报错）。cursor 固定 datasetVersion，翻页期间数据更新不影响本系列结果；版本被清理时会返回 dataset_version_expired，需从第一页重查');
const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;
const CHANGES_SCHEMA = {
    provider: providerField,
    type: z.enum(['source_observation', 'source', 'price_change', 'price']).optional().describe('source_observation=来源页面观察（页面内容变化，不等于模型发布/下线）；price_change=价格事件'),
    q: qField.describe('标题/摘要文本包含匹配'),
    from: z.string().regex(DATE_RE).optional().describe('窗口起点（含），YYYY-MM-DD，必须与 to 成对；不传时默认最近 7 个上海日历日（锚定数据 dataThrough，不是今天）'),
    to: z.string().regex(DATE_RE).optional().describe('窗口终点（不含），窗口 [from,to)，最大 90 天'),
    limit: limitField,
    cursor: cursorField,
    includeWithdrawn: z.boolean().optional().describe('true 时包含已撤回条目（默认隐藏；已撤回条目详情仍可按 id 查询）'),
};
const PRICES_SCHEMA = {
    model: z.string().optional().describe('模型名精确匹配（大小写不敏感、规范化后），无别名推断。要「找类似名字的模型」请改用 q'),
    provider: providerField,
    component: z.string().optional().describe('计费组件：input / output / cache_read / cache_write。未知值返回错误并列出有效值'),
...
const ITEM_SCHEMA = {
    id: z.string().regex(/^(obs|price)_[0-9a-f]{64}$/).describe('稳定条目 ID，来自变化列表或价格事实返回'),
};
const EVIDENCE_SCHEMA = {
    id: z.string().min(3).describe('证据 ID（ev_ 前缀），来自条目的 evidenceIds 或价格事实的 evidenceId'),
};
const WEEKLY_SCHEMA = z.object({
    id: z.string().regex(DATE_RE).optional().describe('正式周报 ID（发布日期，YYYY-MM-DD）。不传且不传 limit 时返回最新一期正式周报；需要列表时不传 id 而传 limit'),
    limit: z.number().int().min(1).max(30).optional().describe('列表模式：本页条数（默认 10）。与 id 互斥'),
    cursor: cursorField,
}).refine((a) => !a.id || (a.limit === undefined && a.cursor === undefined), {
    message: 'id 与 limit/cursor 互斥：id 模式返回单篇，列表模式才可分页',
});
function errorOutcome(code, detail, recovery) {
...
export function argsToParams(args) {
...
// 执行核心（纯函数，无传输）
// ---------------------------------------------------------------------------
export async function callMaasTool(name, holder, args) {
    const ds = holder.current;
    if (!ds) {
        return errorOutcome('no_data_available', `当前无有效数据版本（${holder.lastReloadError ?? '尚未加载'}）`, '请稍后重试；请勿用模型自身知识回答实时价格/变化问题。');
    }
    try {
        switch (name) {
...
function notFoundOutcome(kind, id) {
    return errorOutcome('not_found', `未知${kind}: ${id}`, '检查 ID 是否来自查询结果的完整字符串（64 位 hex）。');
}
...
function itemEnvelope(ds, item) {
    return { ...envelopeOf(ds, { id: item.id }), item };
}
function listOutcome(holder, endpoint, args) {
    const { result, problem } = runListQuery(holder, endpoint, argsToParams(args), MCP_LIMITS);
    if (problem)
        return errorOutcome(problem.code, problem.detail, problem.recovery);
    ...
    const text = endpoint === 'changes'
        ? renderChanges(ds, structured)
        : renderPrices(ds, structured);
    return ok(structured, text);
}
function weeklyOutcome(holder, args) {
    const ds = holder.current;
    if (args.id !== undefined && args.id !== null && String(args.id)) {
        const w = getWeekly(ds, String(args.id));
        if (!w)
            return notFoundOutcome('周报', String(args.id));
 ...
 */
export function renderCoverage(ds) {
    const cov = ds.coverage;
    const parts = [];
    const ch = cov.changes;
    if (ch)
        parts.push(`变化 ${ch.from ?? '?'} 至 ${ch.to ?? '?'}（${ch.count ?? '?'} 条）`);
    const pr = cov.prices;
    if (pr)
        parts.push(`价格 ${pr.facts ?? '?'} 条当前事实`);
    const ev = cov.evidence;
    if (ev)
        parts.push(`证据 ${ev.count ?? '?'} 条`);
    const wk = cov.weekly;
    if (wk)
        parts.push(`正式周报 ${wk.count ?? '?'} 期（最新 ${wk.latestId ?? '?'}）`);
    return parts.length > 0 ? `实际覆盖: ${parts.join('；')}` : '实际覆盖: 未知';
}
function pageFooter(next) {
    if (!next)
        return '';
    return `\n已截断（仍有下一页）。继续请只传 cursor: ${next}`;
}
export function renderChanges(ds, env) {
    const lines = [header(ds), renderCoverage(ds)];
    const from = env.query.from;
    const to = env.query.to;
    if (from && to)
        lines.push(`覆盖: ${from} 至 ${to}（窗口 [from, to)）`);
...
export function renderPrices(ds, env) {
    const lines = [header(ds), renderCoverage(ds)];
    if (env.items.length === 0) {
        lines.push('该覆盖范围内未记录到匹配项。');
        lines.push(`查询条件: ${JSON.stringify(env.query)}。`);
...
export function renderWeeklyItem(w) {
    const lines = [
        `正式周报 ${w.id}：${w.title}`,
        `周期: ${w.period ?? '未标注'}`,
    ...
    if (Array.isArray(headlines) && headlines.length > 0) {
        ...
        if (headlines.length > 8)
            lines.push(`  …共 ${headlines.length} 条（完整结构见 structuredContent）`);
    }
    return lines.join('\n');
}
function renderWeeklyList(env) {
    const lines = ['正式周报列表（最新在前）:'];
...
export function registerMaasTools(server, holder) {
    ...
    }, async (args) => toResult(await callMaasTool('maas_get_changes', holder, args)));
    ...
    }, async (args) => toResult(await callMaasTool('maas_get_weekly', holder, args)));
}
